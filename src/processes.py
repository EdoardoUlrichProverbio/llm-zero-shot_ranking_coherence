...
def _process_prompts_in_batches(onnx_file_path: str, tokenizer: PreTrainedTokenizer, prompts: List[str], batch_size: int, max_new_tokens: int) -> List[str]:
    """
    Process a batch of prompts using the ONNX model.d
    
    Args:
        onnx_file_path (str): The path to the ONNX model file.
        tokenizer (PreTrainedTokenizer): The tokenizer to process inputs.
        prompts (List[str]): List of input prompts.
        batch_size (int): Number of prompts to process in each batch.
        max_new_tokens (int): Maximum number of new tokens to generate.
    
    Returns:
        List[str]: List of outputs generated by the model.
    """
    # Load the ONNX model (quantized model or not)
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        # Tokenize and process prompts in batch
        inputs = tokenizer(batch_prompts, return_tensors="np", padding=True, truncation=True, max_length=tokenizer.model_max_length)
        
        # Prepare input for ONNX runtime (convert to numpy)
        input_ids: np.ndarray = np.array(inputs["input_ids"], dtype=np.int64)

        # Run inference with ONNX
        logits: np.ndarray = session.run([output_name], {input_name: input_ids})[0]

        # Decoding the logits into text
        batch_outputs: List[str] = tokenizer.batch_decode(np.argmax(logits, axis=-1), skip_special_tokens=True)
